...
import { getDb, initializeDb, runMigrations, uploadDbToS3 } from './helpers.js';
...
app.post('/migrations/:direction', async (req, res) => {
    const db = req.db;
    if (!db) {
        return res.status(500).json({ error: 'Database connection not found' });
    }
...
app.delete('/migrations/:name', async (req, res) => {
    const db = req.db;
    if (!db) {
        return res.status(500).json({ error: 'Database connection not found' });
    }
