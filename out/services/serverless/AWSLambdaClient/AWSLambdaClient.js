import { LambdaClient, CreateFunctionCommand, UpdateFunctionCodeCommand, DeleteFunctionCommand, InvokeCommand, GetFunctionCommand, AddPermissionCommand, RemovePermissionCommand, UpdateFunctionConfigurationCommand, } from '@aws-sdk/client-lambda';
import { IAMClient, CreateRoleCommand, AttachRolePolicyCommand, DetachRolePolicyCommand, DeleteRoleCommand, GetRoleCommand, ListAttachedRolePoliciesCommand, } from '@aws-sdk/client-iam';
import { APIGatewayClient, CreateRestApiCommand, GetResourcesCommand, CreateResourceCommand, PutMethodCommand, PutIntegrationCommand, CreateDeploymentCommand, DeleteRestApiCommand, GetRestApisCommand, PutRestApiCommand, UpdateMethodCommand, PutIntegrationResponseCommand, PutMethodResponseCommand, } from '@aws-sdk/client-api-gateway';
...
export class AWSLambdaClient {
    ...
    async deleteApiGateway(service) {
        try {
            ...
            console.log(`Removed API Gateway permission from Lambda function ${service.name}`);
        }
        catch (error) {
    ...
    async deleteRole() {
        try {
            ...
            const listPoliciesResponse = await this.iamClient.send(listPoliciesCommand);
            // Detach all policies
    ...
    async waitForFunctionActive(name, maxWaitTime = 60000, checkInterval = 5000) {
    ...
    async createOrUpdateFunction(config) {
        const { name, code, handler = 'index.handler', runtime = 'nodejs18.x', environment = {}, } = config;
        if (!this.roleArn) {
        ...
        if (!this.roleArn) {
            throw new Error('Role ARN is not available. Failed to create or retrieve role.');
        }
        try {
            ...
            if (exists) {
                ...
                if (Object.keys(environment).length > 0) {
                    ...
                    await this.lambdaClient.send(new UpdateFunctionConfigurationCommand(updateEnvParams));
                }
            }
            else {
    ...
    async checkApiEndpoint(functionName) {
        try {
            const getApisCommand = new GetRestApisCommand({});
            const apisResponse = await this.apiGatewayClient.send(getApisCommand);
            const api = apisResponse.items?.find((api) => api.name === `${functionName}-api`);
            if (!api || !api.id) {
                return null;
            }
            this.apiId = api.id;
    ...
    async createAPIGateway(functionName) {
