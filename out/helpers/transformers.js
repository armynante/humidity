 ...
 */
export const buildProjectChoices = (projects) => {
    try {
...
export const projectTable = (project) => {
    ...
    table.push({ 'Project ID': project.id }, { Name: project.name }, { Type: project.type }, { Description: project.description }, { Created: created.toLocaleString() }, { Updated: updated.toLocaleString() }, { 'GitHub Repo': project.gitHubRepo }, { 'DigitalOcean Link': project.do_link }, { 'DigitalOcean App ID': project.do_app_id });
...
export const ServiceTable = (service) => {
    ...
    table.push({ 'Service ID': service.id }, { Name: service.name }, { Type: service.serviceType }, { URL: service.url }, { Created: created.toLocaleString() }, { Updated: updated.toLocaleString() });
    return table.toString();
};
export const showErrorAndExit = (message) => {
...
export function formatSummary(project) {
    ...
    const formattedLines = labels.map((label, i) => `${label.padStart(maxLength, ' ')}: ${values[i]}`);
    const summaryHeader = '='.repeat(Math.max(...formattedLines.map((line) => line.length)));
    return [
        summaryHeader,
        '//// Project Summary',
        summaryHeader,
