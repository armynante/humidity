...
export default class GitHub {
   ...
   */
  async encryptSecret(secret: string, repo: string): Promise<[string, string]> {
    if (!this.client) {
      throw new Error('Client not initialized');
    }
    ...
    const secret_encrypted = sodium.to_base64(
      encBytes,
      sodium.base64_variants.ORIGINAL,
    );
  ...
  async uploadRepoSecret(secret: GHSecret) {
    ...
    const [secret_encrypted, key_id] = await this.encryptSecret(
