...
export async function checkDbExistsInS3(
  s3: S3Client,
  bucketName: string,
  dbFileName: string,
): Promise<boolean> {
  try {
    await s3.send(
      new HeadObjectCommand({ Bucket: bucketName, Key: dbFileName }),
    );
    return true;
  } catch (error) {
...
export async function downloadDbFromS3(
  s3: S3Client,
  bucketName: string,
  dbFileName: string,
  localDbPath: string,
...
export async function getDb(
  ...
  if (dbExists) {
    ...
    throw new Error(
      'Database not found in S3. Please initialize the database first.',
    );
  }
  return open({
    filename: localDbPath,
